...
domain = domain_mgr(filename='clad.k')
...
young1 = cp.array(np.loadtxt('../0_properties/IN718_Young_Debroy.txt')[:,1]/1e6)
temp_young1 = cp.array(np.loadtxt('../0_properties/IN718_Young_Debroy.txt')[:,0])
Y1 = cp.array(np.loadtxt('../0_properties/IN718_Yield_Debroy.txt')[:,1]/1e6*np.sqrt(2/3))
temp_Y1 = cp.array(np.loadtxt('../0_properties/IN718_Yield_Debroy.txt')[:,0])
scl1 = cp.array(np.loadtxt('../0_properties/IN718_Alpha_Debroy.txt')[:,1])
temp_scl1 = cp.array(np.loadtxt('../0_properties/IN718_Alpha_Debroy.txt')[:,0])

young2 = cp.array(np.loadtxt('../0_properties/TI64_Young_Debroy.txt')[:,1]/1e6)
temp_young2 = cp.array(np.loadtxt('../0_properties/TI64_Young_Debroy.txt')[:,0])
Y2 = cp.array(np.loadtxt('../0_properties/TI64_Yield_Debroy.txt')[:,1]/1e6*np.sqrt(2/3))
temp_Y2 = cp.array(np.loadtxt('../0_properties/TI64_Yield_Debroy.txt')[:,0])
scl2 = cp.array(np.loadtxt('../0_properties/TI64_Alpha_Debroy.txt')[:,1])
temp_scl2 = cp.array(np.loadtxt('../0_properties/TI64_Alpha_Debroy.txt')[:,0])

poisson =  0.3                        # Poisson's ratio
a1 = 10000
young = cp.zeros((n_e,n_q))
scl = cp.zeros((n_e,n_q))
a  = cp.zeros((n_e,n_q))
Y = cp.zeros((n_e,n_q))
...
idirich = cp.array(domain.nodes[:, 2] == -11.0 ) 
...
last_mech_time = 0
output_timestep = 2
filename = 'results/clad_{}.vtk'.format(file_num)
save_vtk(filename)
file_num = file_num + 1
...
while domain.current_time<endtime-domain.dt:
    ...
    if domain.current_time >= last_mech_time + implicit_timestep:
        ...
        mat = 1
        ele_ID = domain.element_mat == mat
        young[ele_ID] = cp.interp(temperature_ip[ele_ID],temp_young1,young1)
        scl[ele_ID] = cp.interp(temperature_ip[ele_ID],temp_scl1,scl1)
#         a[ele_ID]  = cp.interp(temperature_ip[ele_ID],temp_a1,a1)
        Y[ele_ID] = cp.interp(temperature_ip[ele_ID],temp_Y1,Y1)
        
        mat = 2
        ele_ID = domain.element_mat == mat
        young[ele_ID] = cp.interp(temperature_ip[ele_ID],temp_young2,young2)
        scl[ele_ID] = cp.interp(temperature_ip[ele_ID],temp_scl2,scl2)
#         a[ele_ID]  = cp.interp(temperature_ip[ele_ID],temp_a2,a2)
        Y[ele_ID] = cp.interp(temperature_ip[ele_ID],temp_Y2,Y2)
        
        alpha_Th[:,:,0:3] = scl[:,:,cp.newaxis].repeat(3,axis=2)
        shear = young/(2*(1+poisson))        # shear modulus
        bulk = young/(3*(1-2*poisson))       # bulk modulus
        a = cp.ones_like(young)*a1
